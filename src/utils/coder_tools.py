...
@log_duration
def sub_figma_2_coder(sub_figma_json: dict, icon_list: dict[str, str], task_id: int = 10000) -> str:
    """
    Generates or fixes compose ui code.
    """
    tlogger(task_id).info("---SUB FIGMA CODING ---")
    ...
    if icon_list:
        exported_icons_prompt += "These resources are stored in a dictionary where:\n Keys: Figma identifiers including node IDs, image references (imageRef), and component IDs\nValues: Corresponding file paths and names:\n"
        for icon in icon_list:
            exported_icons_prompt += f"- {str(icon)}\n"
        user_prompt += "\n# Icon List is dict\n" + exported_icons_prompt
    tlogger(task_id).info("generate code start")
    ...
    for retry_time in range(d2c_config.MAXCoderRetry):
        ...
        if coder_output and coder_output.compose_code and d2c_utils.is_valid_sub_compose_code(coder_output.compose_code.strip()):
            tlogger(task_id).info(f"generate code as follows: \n{coder_output.compose_code}")
            break
        else:
            tlogger(task_id).info(f"generate code failed, retry {retry_time + 1} times")
    else:
        tlogger(task_id).info(f"generate code failed, retry {d2c_config.MAXCoderRetry} times, output is empty")
    ...
    if idx > 0:
        tlogger(task_id).info(f"remove from {idx} extra code : {generated_compose_code[:idx]}")
